...
if ( ! class_exists( 'Classic_Dog_Breeder_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Classic_Dog_Breeder_WPTT_WebFont_Loader {
		...
		public function get_remote_url_contents() {
			...
			$classic_dog_breeder_contents = wp_remote_retrieve_body( $response );

			return $classic_dog_breeder_contents;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Classic_Dog_Breeder_WPTT_WebFont_Loader( $url );
